...
import pandas as pd
import os
os.chdir("D:\FinTech_2022\+ 最终提交")
d = pd.read_csv('factor_file_hyk_0626_ff.csv')
d = d.set_index('Date')
#%%
d_ = d.unstack().reset_index()
print(d_)

#%%
d_.columns = ['Stkcd','Date','score']

#%%
d_['rank'] = d_.groupby(['Date'])['score'].rank(ascending = False)
# print(df_predict)

d_200 = d_[d_['rank']<=200].reset_index(drop=True)

print(d_200)

#%%
d_200 = d_200.sort_values(by = ["Date", "rank"], ascending = [True,True]).reset_index(drop=True)


d_200[['Stkcd', 'Date']].to_csv('持仓文件.csv',encoding='utf_8_sig') 
...
d_200['Date'] = d_200.Date.apply(lambda x: fun1(x))
...
d_200['Date'] = d_200.Date.apply(lambda x: fun2(x))

#%%
# d_200['Date'] = 
# d_200['Date'] = d_200.Date.apply(lambda x: int(x[0:4]+x[5:7]+x[8:10]))
# df_['Date'] = df_.Date.apply(lambda x: int(x[0:4]+x[5:7]+x[8:10]))

#%%
df_['Stkcd'] = df_.Stkcd.apply(lambda x: str(x))
...
d_ = d.unstack().reset_index()
d_.columns = ['Stkcd','Date','score']
# d_['score'] = d_['score'].shift(-5)
# d_['rank'] = d_.groupby(['Date'])['score'].rank(ascending = False)
# d_200 = d_[d_['rank']<=200].reset_index(drop=True) # 
d_200 = d_.reset_index(drop=True)
print(d_200)
...
# 重新导入沪深300收盘指数
os.chdir('D:\FinTech_2022') 
bench_1 = pd.read_excel('FUND_IDX_QUOTATIONDAY-1.xlsx')
bench_2 = pd.read_excel('FUND_IDX_QUOTATIONDAY-2.xlsx')
bench = pd.concat([bench_1.iloc[2:,1:],bench_2.iloc[2:,1:]])
bench['RET'] = bench['ClosePrice'] / bench['ClosePrice'].shift(1) - 1
bench = bench.rename(columns = {
    'TradingDate':'Date'})
del bench['ClosePrice']
bench['Stkcd'] = 'benchmark'
bench = bench.iloc[1:,].reset_index(drop=True)
bench = date_change2(bench, 'Date')
...
# 检查键的格式
print(type(df_.iloc[0,0]))
print(type(d_200.iloc[0,0]))
print(type(df_.iloc[0,1])) # 需要调整格式
print(type(d_200.iloc[0,1]))

# 修改格式
d_200['Date'] = d_200.Date.apply(lambda x: fun1(x))
df_['Date'] = df_.Date.apply(lambda x: str(x))
df_['Date'] = df_.Date.apply(lambda x: x[0:10])

# 先把基准分出来
bb = df_[df_['Stkcd'] == "benchmark"].reset_index(drop=True)

# 然后用d_200和df_取交集
dd = pd.merge(df_,d_200,on=['Stkcd','Date'])

# 拆分得到
RR = dd[['Stkcd', 'Date', 'RET']]
FF = dd[['Stkcd', 'Date', 'score']]

RR = RR.sort_values(by = ["Date"]).reset_index(drop=True)
FF = FF.sort_values(by = ["Date"]).reset_index(drop=True)
BB = bb.iloc[1:-6,:].reset_index(drop=True)

# RB = pd.concat([RR,BB])

# 宽面
factor_file = FF.set_index(['Stkcd','Date']).score.unstack(0)


R2 = pd.DataFrame(columns=['Date'],
                    index=np.arange(len(ret_file.index)))
R2['Date'] = factor_file.index  
BB = pd.merge(R2,BB,on=['Date'],how='left')
RB = pd.concat([RR,BB])

# BB_ = BB.set_index(['Stkcd','Date']).RET.unstack(0)

ret_file = RB.set_index(['Stkcd','Date']).RET.unstack(0)
...
# aa.to_csv('分年度情况+.csv',encoding='utf_8_sig') 
# results.to_csv('总体情况+.csv',encoding='utf_8_sig') 

#% 单步测试
rolling_ret,rolling_factors,direct = ret_file,factor_file,1
benchmark = rolling_ret['benchmark']
rolling_ret = rolling_ret.drop(columns='benchmark')
if direct==-1:
    rolling_factors = -rolling_factors
...
#%%
ddd = d_200.iloc[0:4,:2]
ddd['Date'] = ddd.Date.apply(lambda x: int(x[0:4]+x[5:7]+x[8:10]))
...
def fun3(x):
    ...
    y = x[0:4]+"-"+x[4:6]+"-"+x[6:8]
    return y
ddd['Date'] = ddd.Date.apply(lambda x: fun3(x))
